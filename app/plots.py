...
def draw_overview(fig, signals: dict, t0: float = 0.0,
    ...
    for idx, sid in enumerate(ids):
        ...
        if is_digital(info):
            ...
            # Y-limits por percentiles para no dejar la señal aplastada por outliers
            if len(va) >= 4:
...
def draw_compare(fig, sel_ids: list, signals: dict, t0: float = 0.0):
    ...
    # ── 1 señal: dibujo individual con stats ──────────────────────────────────
    if n == 1:
        sid  = valid[0]
        info = signals[sid]
        ts, va = _prep(info, t0)
    ...
    # ── 2 señales: doble eje Y con valores reales ─────────────────────────────
    if n == 2:
    ...
    # ── 3+ señales: superposición normalizada 0-1 ─────────────────────────────
    ax = fig.add_subplot(111)
    _style_ax(ax)
...
def draw_live(fig, ts: list, vals: list, signal_id: int, t0: float = 0.0):
    ...
    # segmento histórico (gris) + segmento reciente (blanco)
