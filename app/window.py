...
QTreeWidget ,QTreeWidgetItem ,QPlainTextEdit ,QTextEdit ,QListWidget ,
QStatusBar ,QProgressBar ,QFileDialog ,QMessageBox ,
QButtonGroup ,QFrame ,QDoubleSpinBox ,QAbstractSpinBox ,
QWidgetAction ,QInputDialog ,QMenu ,QDialog ,QDialogButtonBox ,
)
from PyQt6 .QtCore import Qt ,QThread ,QTimer 
from PyQt6 .QtGui import QColor ,QFont ,QBrush ,QAction ,QDoubleValidator 

from .styles import QSS ,C_BG ,C_CARD ,C_TEXT ,C_HDR ,C_SUB ,C_BORDER ,C_SURF ,C_INPUT ,TYPE_STYLE 
from .analysis import sig_type ,is_digital ,find_duplicate_groups 
...
class TimeSpinBox (QAbstractSpinBox ):
    """Spinbox de tiempo HH:MM:SS con soporte de valores negativos."""

    def __init__ (self ,parent =None ):
        super ().__init__ (parent )
        self ._secs =0 
        self .lineEdit ().setText (self ._fmt (0 ))
    ...
    def _parse (txt :str )->int :
        ...
        try :
            ...
            if len (parts )==3 else int (parts [0 ]))
    ...
    def value (self )->int :
        return self ._parse (self .lineEdit ().text ())

    def setValue (self ,s :int ):
        self ._secs =int (s )
        self .lineEdit ().blockSignals (True )
        self .lineEdit ().setText (self ._fmt (self ._secs ))
    ...
    def stepBy (self ,steps :int ):
        ...
        mult =60 if (mods &Qt .KeyboardModifier .ShiftModifier )else 1 
        self .setValue (self ._parse (self .lineEdit ().text ())+steps *mult )
...
class MainWindow (QMainWindow ):
    ...
    def _build_ui (self ):
        ...
        self ._file_lbl .setObjectName ("subtext")
    ...
    def _build_left (self ):
        ...
        self ._search .setPlaceholderText ("Buscar ID hex, IP...")
        self ._search .textChanged .connect (self ._populate_tree )
        srow .addWidget (self ._search ,1 )
        self ._type_combo =QComboBox ()
        self ._type_combo .addItems (["Todos","float","int","digital"])
        self ._type_combo .setFixedWidth (78 )
        self ._type_combo .setToolTip ("Filtrar por tipo de señal")
        self ._type_combo .currentTextChanged .connect (self ._populate_tree )
        srow .addWidget (self ._type_combo )
        self ._only_var =QCheckBox ("Con variacion")
    ...
    def _build_ind_controls (self ):
        ...
        lay .addWidget (self ._lbl_small ("Offset V:"))
        ...
        lay .addWidget (self ._lbl_small ("Desfase t:"))
        self ._tshift_edit =QLineEdit ("0")
        self ._tshift_edit .setFixedWidth (90 )
        self ._tshift_edit .setPlaceholderText ("ej. -30.5")
        v =QDoubleValidator (-86400.0 ,86400.0 ,3 )
        v .setNotation (QDoubleValidator .Notation .StandardNotation )
        self ._tshift_edit .setValidator (v )
        self ._tshift_edit .setToolTip ("Desplazamiento en tiempo (s). Negativo = adelantar, positivo = retrasar")
        self ._tshift_edit .textChanged .connect (lambda _ :self ._refresh ())
        lay .addWidget (self ._tshift_edit )
        lay .addWidget (self ._lbl_small ("s"))
        ...
        self ._time_edit =TimeSpinBox ()
        self ._time_edit .setFixedWidth (96 )
        self ._time_edit .setToolTip (
        "Hora real de t=0. Acepta HH:MM:SS o -HH:MM:SS. "
        "Flechas = ±1 s, Shift+flechas = ±60 s")
        self ._time_edit .lineEdit ().textChanged .connect (lambda _ :self ._refresh ())
    ...
    def _parse_t0_text (self )->int :
        """Devuelve segundos del campo T₀ (puede ser negativo)."""
        return self ._time_edit .value ()

    def _x_to_time_str (self ,x :float )->str :
        """Convierte x (segundos relativos a t0) en string HH:MM:SS."""
        base_s =self ._parse_t0_text ()
        total_s =int (base_s +x )
        neg =total_s <0 
        abs_s =abs (total_s )%86400 
        h =abs_s //3600 
        m =(abs_s %3600 )//60 
        s =abs_s %60 
    ...
    def _reset_ind_controls (self ):
        ...
        self ._tshift_edit .blockSignals (True )
        self ._tshift_edit .setText ("0")
        self ._tshift_edit .blockSignals (False )
        self ._refresh ()
    ...
    def _fav_key (self ,sid :tuple )->str :
        ...
        base =f"{sid [0 ]}|{sid [1 ]}|{sid [2 ]}|{sid [3 ]}"
        return f"{base }|{sid [4 ]}"if len (sid )>4 and sid [4 ]else base 
    ...
    def _refresh (self ):
        ...
        if m =="overview":
            ...
            if self ._current_id and self ._current_id in self ._signals :
                ...
                try :
                    tshift =float (self ._tshift_edit .text ())
                except ValueError :
                    tshift =0.0 
                if tshift !=0.0 :
                    info =dict (info )
                    info ["ts"]=[t +tshift for t in info ["ts"]]
    ...
    def _on_tree_context_menu (self ,pos ):
        ...
        act_decode =menu .addAction ("Configurar decodificacion...")
        ...
        elif chosen ==act_decode :
            self ._config_decode_dialog (sid )
        elif chosen ==act_csv :
            self ._current_id =sid 
    ...
    def _frame_bytes_html (hex_str :str )->str :
        ...
        if len (data )>=12 and data [1 :4 ]==b'\x00\x00\x00':
            ...
            if rs and len (data )>=rs :
                for i in range (4 ):
                    parts .append (span (data [i ],'#161616','#505050'))
                for i in range (4 ,8 ):
                    parts .append (span (data [i ],'#1a0830','#b060e8'))
                for i in range (8 ,10 ):
                    parts .append (span (data [i ],'#001530','#5090d8'))
                parts .append (span (data [10 ],'#001800','#40c040'))
                st_fg ='#e04040'if data [11 ]==0x57 else '#f0a830'
                parts .append (span (data [11 ],'#1e1000',st_fg ))
                for i in range (12 ,min (rs ,len (data ))):
                    parts .append (span (data [i ],'#200a0a','#e07060'))
                return ' '.join (parts )
        ...
        if len (data )>=8 and data [0 ]==0x01 and data [2 ]>=5 :
            ...
            if len (data )>=block_size :
                parts .append (span (data [0 ],'#1e1600','#e8b830'))
                parts .append (span (data [1 ],'#001530','#5090d8'))
                parts .append (span (data [2 ],'#001800','#40c040'))
                parts .append (span (data [3 ],'#141414','#383838'))
                for i in range (4 ,8 ):
                    parts .append (span (data [i ],'#1a0830','#b060e8'))
                vsize =dlen -4 
                ...
                for i in range (8 +vsize ,block_size ):
                    parts .append (span (data [i ],'#141414','#383838'))
                return ' '.join (parts )
    ...
    @staticmethod 
    def _frame_bytes_selection_html (raw_hex :str ,
    sel_offset :int ,sel_size :int )->str :
        """
        Igual que _frame_bytes_html pero los bytes [sel_offset : sel_offset+sel_size]
        se resaltan como selección activa (blanco/verde), y los restantes del campo
        Value quedan en gris oscuro.
        """
        from .parser import _tcp_rec_size 
        try :
        ...
        def span (b ,bg ,fg ,extra =''):
            return (f'<span style="{BS }background:{bg };color:{fg };{extra }">'
            f'{b :02X}</span>')

        def span_sel (b ,i ):
            if sel_offset <=i <sel_offset +sel_size :
                return span (b ,'#1a4a1a','#60e060',
                'outline:1px solid #40c040;font-weight:bold;')
            return span (b ,'#181818','#404040')
        ...
        if len (data )>=12 and data [1 :4 ]==b'\x00\x00\x00':
            ...
            if rs and len (data )>=rs :
                ...
                parts .append (span (data [11 ],'#1e1000',st_fg ))
                for i in range (12 ,min (rs ,len (data ))):
                    parts .append (span_sel (data [i ],i ))
                return ' '.join (parts )
        ...
        for i ,b in enumerate (data ):
            parts .append (span_sel (b ,i ))
        return ' '.join (parts )

    def _config_decode_dialog (self ,sid ):
        from .parser import decode_with_override 
        ...
        payloads =info .get ("payloads",[])
        ts_list =info .get ("ts",[])
        val_list =info .get ("val",[])
        fk =self ._fav_key (sid )
        if fk not in self ._favorites :
            self ._favorites [fk ]={"name":"","note":""}
        cur =self ._favorites [fk ].get ("override",{})
        cur2 =cur .get ("ch2",{})

        dlg =QDialog (self )
        dlg .setWindowTitle (f"Decodificacion — 0x{sig_id :04X} ({sig_id })")
        dlg .resize (900 ,720 )
        root =QHBoxLayout (dlg )
        root .setSpacing (6 )


        left =QWidget ();left .setMaximumWidth (330 )
        llay =QVBoxLayout (left );llay .setSpacing (5 );llay .setContentsMargins (0 ,0 ,0 ,0 )

        hdr_lbl =QLabel (
        f"<b>0x{sig_id :04X}</b>  {transport }  "
        f"{info .get ('ip_src','')}→{info .get ('ip_dst','')}  "
        f"<span style='color:#606060'>{len (payloads )} tramas</span>")
        hdr_lbl .setTextFormat (Qt .TextFormat .RichText )
        hdr_lbl .setObjectName ("subtext")
        llay .addWidget (hdr_lbl )
        ...
        for i ,raw in enumerate (payloads ):
            ...
            v =val_list [i ]if i <len (val_list )else 0 
            t_rel =ts -self ._t0 
            nb =len (raw .replace (' ',''))//2 
            vs =f"{v :.5g}"if info ["is_float"]else str (v )
            lst .addItem (f"[{i +1 :4d}] +{t_rel :8.2f}s  {vs :<12} ({nb }B)")
        llay .addWidget (lst )


        frame_view =QTextEdit ();frame_view .setReadOnly (True )
        frame_view .setMaximumHeight (58 );frame_view .setFont (QFont ("Consolas",10 ))
        frame_view .setStyleSheet (
        f"background:{C_INPUT };border:1px solid {C_BORDER };border-radius:4px;padding:3px;")
        llay .addWidget (frame_view )
        legend_lbl =QLabel ();legend_lbl .setTextFormat (Qt .TextFormat .RichText )
        llay .addWidget (legend_lbl )
        byte_dir_lbl =QLabel ();byte_dir_lbl .setTextFormat (Qt .TextFormat .RichText )
        byte_dir_lbl .setFont (QFont ("Consolas",8 ))
        llay .addWidget (byte_dir_lbl )

        llay .addWidget (self ._sep ())


        chk =QCheckBox ("Usar decodificacion manual")
        chk .setChecked (bool (cur ))
        llay .addWidget (chk )

        ctrl1 =QWidget ()
        r1 =QHBoxLayout (ctrl1 );r1 .setSpacing (6 );r1 .setContentsMargins (0 ,0 ,0 ,0 )
        r1 .addWidget (self ._lbl_small ("Off:"))
        spn_off =QSpinBox ();spn_off .setRange (0 ,16 );spn_off .setValue (cur .get ("offset",0 ))
        spn_off .setFixedWidth (46 );spn_off .setToolTip ("Offset dentro del campo valor (bytes)")
        r1 .addWidget (spn_off )
        r1 .addWidget (self ._lbl_small ("B:"))
        cmb_bytes =QComboBox ();cmb_bytes .addItems (["1","2","4","8"])
        cmb_bytes .setCurrentText (str (cur .get ("vsize",4 )));cmb_bytes .setFixedWidth (46 )
        r1 .addWidget (cmb_bytes )
        r1 .addWidget (self ._lbl_small ("Tipo:"))
        cmb_type =QComboBox ();cmb_type .addItems (["float","uint","int"])
        cmb_type .setCurrentText (cur .get ("dtype","float"));cmb_type .setFixedWidth (58 )
        r1 .addWidget (cmb_type )
        cmb_end =QComboBox ();cmb_end .addItems (["LE","BE"])
        cmb_end .setCurrentText (cur .get ("endian","LE"));cmb_end .setFixedWidth (46 )
        r1 .addWidget (cmb_end )
        llay .addWidget (ctrl1 )

        res_lbl =QLabel ();res_lbl .setTextFormat (Qt .TextFormat .RichText )
        llay .addWidget (res_lbl )


        grp2 =QGroupBox ("Canal 2 (split)")
        grp2 .setCheckable (True );grp2 .setChecked (bool (cur2 ))
        g2lay =QHBoxLayout (grp2 );g2lay .setSpacing (6 )
        g2lay .addWidget (self ._lbl_small ("Off:"))
        spn_off2 =QSpinBox ();spn_off2 .setRange (0 ,16 );spn_off2 .setValue (cur2 .get ("offset",4 ))
        spn_off2 .setFixedWidth (46 );g2lay .addWidget (spn_off2 )
        g2lay .addWidget (self ._lbl_small ("B:"))
        cmb_bytes2 =QComboBox ();cmb_bytes2 .addItems (["1","2","4","8"])
        cmb_bytes2 .setCurrentText (str (cur2 .get ("vsize",4 )));cmb_bytes2 .setFixedWidth (46 )
        g2lay .addWidget (cmb_bytes2 )
        cmb_type2 =QComboBox ();cmb_type2 .addItems (["float","uint","int"])
        cmb_type2 .setCurrentText (cur2 .get ("dtype","float"));cmb_type2 .setFixedWidth (58 )
        g2lay .addWidget (cmb_type2 )
        cmb_end2 =QComboBox ();cmb_end2 .addItems (["LE","BE"])
        cmb_end2 .setCurrentText (cur2 .get ("endian","LE"));cmb_end2 .setFixedWidth (46 )
        g2lay .addWidget (cmb_end2 )
        llay .addWidget (grp2 )

        llay .addWidget (self ._sep ())


        llay .addWidget (self ._lbl_small ("Primeras 10 muestras:"))
        preview =QPlainTextEdit ();preview .setReadOnly (True )
        preview .setFont (QFont ("Consolas",8 ));preview .setMaximumHeight (120 )
        llay .addWidget (preview )
        llay .addStretch ()
        ...
        tab_main =QWidget ()
        tm_lay =QVBoxLayout (tab_main );tm_lay .setContentsMargins (4 ,4 ,4 ,4 )
        fig_main =Figure (figsize =(5 ,3.5 ),dpi =90 )
        fig_main .patch .set_facecolor ('#0d0d0d')
        canvas_main =FigureCanvas (fig_main )
        tm_lay .addWidget (canvas_main ,1 )
        tabs .addTab (tab_main ,"Grafica")


        tab_all =QWidget ()
        ta_lay =QVBoxLayout (tab_all );ta_lay .setContentsMargins (4 ,4 ,4 ,4 )
        fig_all =Figure (figsize =(5 ,3.5 ),dpi =90 )
        fig_all .patch .set_facecolor ('#0d0d0d')
        canvas_all =FigureCanvas (fig_all )
        ta_lay .addWidget (canvas_all ,1 )
        tabs .addTab (tab_all ,"Todas las interpretaciones")


        def _decode_series (vsize ,dtype ,endian ,extra_off ):
            ts_arr ,val_arr =[],[]
            for i ,raw in enumerate (payloads ):
                v ,_ =decode_with_override (raw ,transport ,vsize ,dtype ,endian ,extra_off )
                if v is not None :
                    ts_arr .append ((ts_list [i ]-self ._t0 )if i <len (ts_list )else i )
                    val_arr .append (v )
            return np .array (ts_arr ),np .array (val_arr ,dtype =float )

        def _plot_ax (ax ,ts ,vals ,label ,color ='#d4d4d4'):
            ...
            if len (vals )==0 :
                ...
                ha ='center',va ='center',color ='#404040',fontsize =8 )
                ax .set_ylim (0 ,1 )
                return 
            ...
            try :
                ...
                if len (finite )>=2 :
                    ...
                    pad_y =max (float (p99 -p1 )*0.15 ,1e-9 )
                    if np .isfinite (p1 )and np .isfinite (p99 ):
                        ax .set_ylim (float (p1 )-pad_y ,float (p99 )+pad_y )
                        vals =np .clip (vals ,float (p1 )-pad_y *5 ,
                        float (p99 )+pad_y *5 )
            except Exception :
                pass 
            ax .plot (ts ,vals ,color =color ,lw =0.9 ,zorder =3 )
            ax .set_title (label ,color ='#b0b0b0',fontsize =8 ,pad =3 )
            try :
                mn ,mx =float (np .nanmin (vals )),float (np .nanmax (vals ))
                ax .set_ylabel (f"[{mn :.3g}, {mx :.3g}]",color ='#505050',fontsize =7 )
            except Exception :
                pass 

        def refresh_main ():
            active =chk .isChecked ()
            ctrl1 .setEnabled (active )
            fig_main .clear ()
            if not payloads :
                canvas_main .draw ();return 

            has_ch2 =grp2 .isChecked ()and active 
            if has_ch2 :
                ax1 =fig_main .add_subplot (211 )
                ax2 =fig_main .add_subplot (212 )
            else :
                ax1 =fig_main .add_subplot (111 )
                ax2 =None 

            vsize =int (cmb_bytes .currentText ())
            dtype =cmb_type .currentText ()
            endian =cmb_end .currentText ()
            extra =spn_off .value ()

            if active :
                ts1 ,v1 =_decode_series (vsize ,dtype ,endian ,extra )
                lbl1 =f"{dtype }{vsize *8 } {endian }  off+{extra }"
            else :
                ts1 =np .array ([(t -self ._t0 )for t in ts_list ])
                v1 =np .array (val_list ,dtype =float )
                lbl1 ="auto"
            _plot_ax (ax1 ,ts1 ,v1 ,lbl1 ,'#8ab4e8')

            if ax2 is not None :
                v2sz =int (cmb_bytes2 .currentText ())
                v2typ =cmb_type2 .currentText ()
                v2end =cmb_end2 .currentText ()
                v2off =spn_off2 .value ()
                ts2 ,v2 =_decode_series (v2sz ,v2typ ,v2end ,v2off )
                _plot_ax (ax2 ,ts2 ,v2 ,
                f"Canal2: {v2typ }{v2sz *8 } {v2end }  off+{v2off }",'#8ed0a0')

            try :
                fig_main .tight_layout (pad =0.6 )
            except Exception :
                pass 
            canvas_main .draw ()
        ...
        def refresh_all ():
            from .parser import get_all_valid_configs 
            nonlocal _drawn_configs 
            fig_all .clear ()
            _drawn_configs =[]
            if not payloads :
                canvas_all .draw ()
            ...
            if not configs :
                ax =fig_all .add_subplot (111 )
                ax .set_facecolor ('#111111')
                ax .text (0.5 ,0.5 ,'sin tramas o bytes insuficientes',
                transform =ax .transAxes ,ha ='center',va ='center',
                color ='#505050',fontsize =10 )
                canvas_all .draw ()
                return 

            _colors ={"float":"#8ab4e8","uint":"#8ed0a0","int":"#e09090"}
            n =len (configs )
            cols =3 
            rows =(n +cols -1 )//cols 
            fig_all .set_size_inches (5 ,max (2.5 ,rows *1.8 ))

            for idx ,(label ,vsize ,dtype ,endian ,extra )in enumerate (configs ):
                ax =fig_all .add_subplot (rows ,cols ,idx +1 )
                ts_a ,v_a =_decode_series (vsize ,dtype ,endian ,extra )
                color =_colors .get (dtype ,"#d4d4d4")
                ax .set_facecolor ('#0e0e0e')
                ax .tick_params (colors ='#505050',labelsize =6 )
                for sp in ax .spines .values ():
                    sp .set_color ('#1e1e1e')
                if len (v_a )==0 :
                    ax .text (0.5 ,0.5 ,'sin datos',transform =ax .transAxes ,
                    ha ='center',va ='center',color ='#303030',fontsize =7 )
                    ax .set_ylim (0 ,1 )
                else :

                    try :
                        finite =v_a [np .isfinite (v_a )]
                        if len (finite )>=2 :
                            p1 ,p99 =np .percentile (finite ,[1 ,99 ])
                            rng =float (p99 -p1 )
                            if np .isfinite (p1 )and np .isfinite (p99 )and rng >=0 :
                                pad_y =max (rng *0.15 ,1e-6 )
                                ax .set_ylim (float (p1 )-pad_y ,float (p99 )+pad_y )
                                v_plot =np .clip (v_a ,float (p1 )-pad_y *5 ,
                                float (p99 )+pad_y *5 )
                            else :
                                v_plot =v_a 
                        else :
                        ...
                        v_plot =v_a 
                    ax .plot (ts_a ,v_plot ,color =color ,lw =0.8 ,zorder =3 )
                    try :
                        mn =float (np .nanmin (v_a ))
                        mx =float (np .nanmax (v_a ))
                        ax .set_xlabel (f"{mn :.3g} … {mx :.3g}",
                        color ='#505050',fontsize =6 ,labelpad =1 )
                    except Exception :
                        pass 
                ax .set_title (f"{label }  ↵",color ='#a0a0a0',fontsize =7 ,pad =2 )
                _drawn_configs .append ((label ,vsize ,dtype ,endian ,extra ))

            try :
                fig_all .tight_layout (pad =0.3 ,h_pad =0.8 ,w_pad =0.4 )
            except Exception :
                pass 
            canvas_all .draw ()

        def _on_all_click (event ):
            if event .inaxes is None :
                return 
            axes_list =fig_all .axes 
            for idx ,ax in enumerate (axes_list ):
                if ax is event .inaxes and idx <len (_drawn_configs ):
                    label ,vsize ,dtype ,endian ,extra =_drawn_configs [idx ]

                    for a in axes_list :
                        a .set_facecolor ('#0e0e0e')
                    ...
                    chk .setChecked (True )
                    cmb_bytes .setCurrentText (str (vsize ))
                    cmb_type .setCurrentText (dtype )
                    cmb_end .setCurrentText (endian )
                    spn_off .setValue (extra )

                    tabs .setCurrentIndex (0 )
                    break 

        canvas_all .mpl_connect ('button_press_event',_on_all_click )

        def refresh_frame (row ):
            if not payloads or row <0 or row >=len (payloads ):
                return 
            raw =payloads [row ]
            active =chk .isChecked ()
            vsize =int (cmb_bytes .currentText ())
            endian =cmb_end .currentText ()
            extra =spn_off .value ()
            base =(8 if transport =="UDP"else 12 )+extra 
            html =self ._frame_bytes_selection_html (
            raw ,base ,vsize if active else 99 )
            frame_view .setHtml (f'<div style="margin:2px">{html }</div>')
            legend_lbl .setText (self ._frame_legend_html (raw ))


            if active and vsize >1 :
                b_start =base 
                b_end =base +vsize -1 
                if endian =="LE":
                    byte_dir_lbl .setText (
                    f'<span style="color:#60e060">B{b_start }</span>'
                    f'<span style="color:#505050">=LSB &nbsp;··· &nbsp;</span>'
                    f'<span style="color:#60e060">B{b_end }</span>'
                    f'<span style="color:#505050">=MSB &nbsp;</span>'
                    f'<span style="color:#e8b830">LE: MSB a la derecha</span>')
                else :
                    byte_dir_lbl .setText (
                    f'<span style="color:#60e060">B{b_start }</span>'
                    f'<span style="color:#505050">=MSB &nbsp;··· &nbsp;</span>'
                    f'<span style="color:#60e060">B{b_end }</span>'
                    f'<span style="color:#505050">=LSB &nbsp;</span>'
                    f'<span style="color:#5090d8">BE: MSB a la izquierda</span>')
            else :
                byte_dir_lbl .setText ('')

            if active :
                val ,_ =decode_with_override (raw ,transport ,vsize ,
                cmb_type .currentText (),
                endian ,extra )
                if val is None :
                    res_lbl .setText ('<span style="color:#e04040">bytes insuficientes</span>')
                else :
                    res_lbl .setText (
                    f'<b style="color:#60e060;font-size:11pt">{val :.9g}</b>'
                    f'<span style="color:#808080">  ({cmb_type .currentText ()}'
                    f'{vsize *8 } {endian } off+{extra })</span>')
            else :
                v =val_list [row ]if row <len (val_list )else None 
                byte_dir_lbl .setText ('')
                if v is not None :
                    res_lbl .setText (
                    f'<b style="color:#b0b0b0;font-size:11pt">{v :.9g}</b>'
                    f'<span style="color:#606060">  (auto)</span>')
        ...
        def refresh_preview ():
            ...
            endian =cmb_end .currentText ()
            extra =spn_off .value ()
            for i ,raw in enumerate (payloads [:10 ]):
                t_r =(ts_list [i ]-self ._t0 )if i <len (ts_list )else i 
                if active :
                    ...
                    tag ="auto"
                vs =f"{v :.7g}"if v is not None else "ERROR"
                lines .append (f"+{t_r :7.2f}s  {vs :<14}  {tag }")
            preview .setPlainText ("\n".join (lines ))

        def full_refresh ():
            refresh_preview ()
            refresh_frame (lst .currentRow ())
        ...
        lst .currentRowChanged .connect (lambda r :refresh_frame (r ))
        tabs .currentChanged .connect (on_tab_changed )
        for w in (chk ,cmb_bytes ,cmb_type ,cmb_end ,cmb_bytes2 ,cmb_type2 ,
        cmb_end2 ,grp2 ):
            if hasattr (w ,'stateChanged'):
                w .stateChanged .connect (lambda _ :full_refresh ())
            elif hasattr (w ,'currentTextChanged'):
                w .currentTextChanged .connect (lambda _ :full_refresh ())
            elif hasattr (w ,'toggled'):
                w .toggled .connect (lambda _ :full_refresh ())
        spn_off .valueChanged .connect (lambda _ :full_refresh ())
        spn_off2 .valueChanged .connect (lambda _ :full_refresh ())
        ...
        btn_row =QHBoxLayout ()
        btn_reset =QPushButton ("Restablecer auto")
        btn_ok =QPushButton ("Aplicar y cerrar")
        ...
        sep_f .setStyleSheet (f"color:{C_BORDER }")

        wrapper =QWidget ();wrapper .setLayout (root )
        main_lay =QVBoxLayout (dlg );main_lay .setContentsMargins (8 ,6 ,8 ,6 )
        main_lay .setSpacing (4 )
        main_lay .addWidget (wrapper ,1 )
        main_lay .addWidget (sep_f )
        main_lay .addLayout (btn_row )

        btn_reset .clicked .connect (lambda :(chk .setChecked (False ),dlg .accept ()))
        ...
        if chk .isChecked ():
            ...
            "dtype":cmb_type .currentText (),
            "endian":cmb_end .currentText (),
            "offset":spn_off .value (),
            }
            if grp2 .isChecked ():
                ovr ["ch2"]={
                "vsize":int (cmb_bytes2 .currentText ()),
                "dtype":cmb_type2 .currentText (),
                "endian":cmb_end2 .currentText (),
                "offset":spn_off2 .value (),
                }
            self ._favorites [fk ]["override"]=ovr 
        else :
            self ._favorites [fk ].pop ("override",None )

        self ._save_favorites ()
        self ._apply_signal_override (sid )

    def _apply_signal_override (self ,sid ):
        """Re-decodifica los payloads de una señal con la config override (o la auto)."""
        from .parser import decode_with_override 
        if sid not in self ._signals :
            return 
        info =self ._signals [sid ]
        ...
        if not payloads :
            return 
        fk =self ._fav_key (sid )
        ovr =self ._favorites .get (fk ,{}).get ("override")

        new_vals =[]
        float_votes =0 
        for raw in payloads :
            if ovr :
                val ,is_f =decode_with_override (
                raw ,info .get ("transport","UDP"),
                ovr ["vsize"],ovr ["dtype"],ovr ["endian"])
            else :
                from .parser import _decode_value 
                try :
                    data =bytes .fromhex (raw .replace (" ","").replace (":",""))
                except ValueError :
                    val ,is_f =None ,False 
                else :
                    offset =8 if info .get ("transport")=="UDP"else 12 
                    dlen =data [2 ]if len (data )>2 else 0 
                    vsize =dlen -4 if info .get ("transport")=="UDP"else 4 
                    val ,is_f =_decode_value (data ,offset ,max (1 ,vsize ))
            if val is not None :
        ...
        info ["val"]=new_vals 
        info ["is_float"]=bool (ovr )and ovr ["dtype"]=="float"if ovr else float_votes >=len (new_vals )//2 
        info ["min"]=float (arr .min ())
        info ["max"]=float (arr .max ())
        info ["std"]=float (arr .std ())
        info ["range"]=float (arr .max ()-arr .min ())
    ...
    def _populate_tree (self ):
        ...
        for sid ,info in ranked :
            ...
            if dst_f and dst !=dst_f :
                continue 
            if type_f =="float"and not info ["is_float"]:
                continue 
            if type_f =="int"and (info ["is_float"]or info ["max"]<=10 ):
                continue 
            if type_f =="digital"and not is_digital (info ):
                continue 
    ...
    def _export_signal_csv (self ):
        ...
        try :
            import csv 
            payloads =info .get ("payloads",[])
            tipo_str =self ._signal_type_str (self ._current_id )
            with open (path ,"w",newline ="",encoding ="utf-8")as f :
                writer =csv .writer (f )
                writer .writerow ([
                "N","timestamp_epoch","tiempo_relativo_s",
                "ip_src","ip_dst","transporte","tipo","valor","trama_hex",
                ])
    ...
    def _export_signals_visible_csv (self ):
        ...
        try :
            ...
            for sid in visible_keys :
                info =self ._signals [sid ]
                payloads =info .get ("payloads",[])
                tipo_str =self ._signal_type_str (sid )
            ...
            with open (path ,"w",newline ="",encoding ="utf-8")as f :
                ...
                for n ,(ts ,sig_id ,val ,raw ,ip_src ,ip_dst ,transport ,tipo_str )in enumerate (rows ,1 ):
                    writer .writerow ([
                    n ,f"0x{sig_id :04X}",
                    f"{ts :.6f}",f"{ts -self ._t0 :.6f}",
                    ip_src ,ip_dst ,transport ,tipo_str ,val ,raw ,
                    ])
            self ._set_status (
    ...
    def _signal_type_str (self ,sid )->str :
        """Devuelve el tipo de decodificación de la señal: float32 LE, uint32 LE, etc."""
        info =self ._signals .get (sid )
        if not info :
            return "?"
        fk =self ._fav_key (sid )
        ovr =self ._favorites .get (fk ,{}).get ("override")
        if ovr :
            return f"{ovr ['dtype']}{ovr ['vsize']*8 } {ovr ['endian']}"

        payloads =info .get ("payloads",[])
        vsize =4 
        if payloads :
            try :
                raw =bytes .fromhex (payloads [0 ].replace (" ","").replace (":",""))
                transport =info .get ("transport","UDP")
                base =8 if transport =="UDP"else 12 
                available =len (raw )-base 
                if available >=8 :
                    vsize =8 
                elif available >=4 :
                    vsize =4 
                ...
                pass 
        if info ["is_float"]:
            return f"float{vsize *8 } LE"
        return f"uint{vsize *8 } LE"
    ...
    def _start_analyze (self ,pcap :str ):
        ...
        self ._favorites ={}
        self ._save_favorites ()
        self ._current_pcap =pcap 
        name =os .path .basename (pcap )
        self .setWindowTitle (f"Ethernet Signal Analyzer — {name }")
    ...
    def _on_done (self ,pkts ,sigs ,t0 ,dur ):
        ...
        self ._time_edit .lineEdit ().blockSignals (True )
        self ._time_edit .setValue (dt .hour *3600 +dt .minute *60 +dt .second )
        self ._time_edit .lineEdit ().blockSignals (False )
